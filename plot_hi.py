...
#Load OFF data
z_off = np.fromfile(obs_off, dtype="float32").reshape(-1, nchan)/nbin
z_off = z_off*10000
...
gs = GridSpec(2,3, height_ratios=[2, 1.5])
...
#def window(size): #average function
#    return np.ones(size)/float(size)

#Plot Calibrated Spectrum
ax2 = fig.add_subplot(gs[0,1])
ax2.plot(freq, estimate_S_N_simple(spectrum, mask), "-b", label="Original")
#ax2.plot(freq, np.convolve(estimate_S_N_simple(spectrum, mask),window(10),'same'), 'r') #average

###
#N = 10
spectrum_median = estimate_S_N_simple(spectrum, mask)

for j in range(0, int(nchan)): #int(nchan-N)
    spectrum_median[j] = np.median(spectrum_median[j:j+N])

ax2.plot(freq, spectrum_median, "-r", label="Median")
###
...
os.rename('freq.txt', 'freq_' + str(i+1) + '.txt')
np.savetxt('snr.txt', spectrum_median, delimiter='\n', fmt='%1.3f')
os.rename('snr.txt', 'snr_' + str(i+1) + '.txt')
np.savetxt('snr_original.txt', estimate_S_N_simple(spectrum, mask), delimiter='\n', fmt='%1.3f')
os.rename('snr_original.txt', 'snr_original' + str(i+1) + '.txt')
...
ax4.plot(t,w)
np.savetxt('power.txt', w, delimiter='\n', fmt='%1.3f')
os.rename('power.txt', 'power_' + str(i+1) + '.txt')
np.savetxt('time.txt', t, delimiter='\n', fmt='%1.3f')
os.rename('time.txt', 'time_' + str(i+1) + '.txt')
